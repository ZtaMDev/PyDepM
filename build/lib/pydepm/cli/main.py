...
def add(
    ...
    if declared is None:
        ...
        console.print(f"[cyan]ℹ[/cyan] Dependency [green]{spec}[/green] already declared")

    if not changed and not use_global:
        raise typer.Exit(code=0)
    ...
    try:
        ...
        with console.status("[bold cyan]Installing dependency...[/bold cyan]", spinner="dots"):
        ...
        if code != 0:
            console.print(f"[red bold]✗ Installation failed[/red bold] (exit code {code})")
            if err:
...
def update(
    spec: str = typer.Argument(
        ..., help="Updated dependency spec, e.g. 'requests>=2.3'.",
    ),
    save_dev: bool = typer.Option(
        False,
        "-D",
    ...
    if not (change.added or change.updated):
        console.print("[yellow]No changes to dependencies (already up-to-date).[/yellow]")
        raise typer.Exit(code=0)
    ...
    try:
        _run_steps_with_progress(
            [
                ("Installing updated dependency...", _pip_install),
                ("Verifying environment (pip check)...", _pip_verify),
            ]
        )
    except subprocess.CalledProcessError as exc:
        ...
        out = (exc.output or "").strip() if isinstance(exc.output, str) else ""
        console.print(
            f"[red bold]Dependency update failed[/red bold] (exit code {exc.returncode})."
        )
        if stderr:
            console.print("[red]--- stderr ---[/red]")
            console.print(stderr)
...
def install(
    ...
    def _resolve_and_install() -> None:
        ...
        # Install using pip (with resolved versions)
        with console.status("[bold cyan]Installing dependencies...[/bold cyan]", spinner="dots"):
            code, out, err = _run_command_with_smooth_progress(
                description="Installing packages",
                args=[python_executable, "-m", "pip", "install", "--quiet", *resolved_specs],
                env=env_vars,
            )
